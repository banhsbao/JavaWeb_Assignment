...
public class SearchLastnameServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String url = listMapping.get(SEARCH_PAGE);
        try {
            String searchValue = request.getParameter("txtSearchValue");
            if(searchValue.length()>0){
                CusDAO dao = new CusDAO();
                dao.searchLastName(searchValue);
                List<CusDTO> listSearching = dao.getListAccounts();
                request.setAttribute("SEARCH_RESULT", listSearching);
            }
        } catch (SQLException sq) {
            log("SearchLastnameServlet_SQLException "+sq.getMessage());
